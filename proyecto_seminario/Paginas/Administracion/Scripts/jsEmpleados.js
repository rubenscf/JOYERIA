...
fCrearEmpleado = function (command,record) {
    if 
    (command == 'editarEmpleado')
        App.direct.fCrearEmpleado(2, record.data.CODIGO);
    else
        App.direct.fCrearEmpleado(1, 0);
...
fCerrarVentanaEmpleado = function () {
    parent.App.direct.fLlenarGrid();
    parent.App.Win_fActualizaEmpleado.close();
...
ObtenerFiltro = function () {
    ...
    f.push({
        filter: function (record) {
            return filtrarCadena(App.FiltroEmpleado.value || "", "TIENDA", record);
        }
    });
